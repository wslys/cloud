...
import (
	...
	user "beaconCloud/rpcServer/user-srv/proto/account"
...
var (
	...
	accountQ = map[string]string{
		...
		"create": `INSERT into %s.%s (
				id, username, email, nodeId, salt, password, created, updated) 
				values (?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		"readByAccount":           "SELECT id, username, email, salt, password, created, updated from %s.%s where username = ?",
		...
		"searchUsernameAndEmail2": "SELECT id, nodeId, username, email, salt, password, created, updated from %s.%s where email = ? limit ? offset ?",
	}
...
func Create(user *user.User, salt string, password string) error {
	...
	_, err := st["create"].Exec(user.Id, user.Username, user.Email, user.NodeId, salt, password, user.Created, user.Updated)
	return err
}

func GetUserByAccount(username string) (*user.User, error) {
	user := &user.User{}
	r := st["readByAccount"].QueryRow(username)
	var s, p string
...
func ReadUserByUsernameAndEmail(email string) (*user.User, error) {
	var r *sql.Rows
	var err error

	r, err = st["searchUsernameAndEmail2"].Query(email, 1, 0)
	...
	var salt, pass string
	user := &user.User{}
	if err := r.Scan(&user.Id, &user.NodeId, &user.Username, &user.Email, &salt, &pass, &user.Created, &user.Updated); err != nil {
		if err == sql.ErrNoRows {
			return nil, errors.New("not found")
		}
		return nil, err
